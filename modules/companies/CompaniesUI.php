...
include_once('./lib/UserAccessValidation.php');
class CompaniesUI extends UserInterface
{
    ...
    public function __construct()
    {
        ...
        $this->_moduleTabText = 'Clients';
        $this->_subTabs = array(
            'Add Clients'     => CATSUtility::getIndexName() . '?m=companies&amp;a=add*al='.ACCESS_LEVEL_EDIT.'*hrmode=0',
            'Search Clients' => CATSUtility::getIndexName() . '?m=companies&amp;a=search*hrmode=0',
            'Go To My Clients' => CATSUtility::getIndexName() . '?m=companies&amp;a=internalPostings*hrmode=0'
        );
    }
    ...
    public function handleRequest()
    {
        ...
        if (!eval(Hooks::get('CLIENTS_HANDLE_REQUEST'))) return;                        switch ($action)
        {
            case 'show':
                $this->show();
                break;
            ...
            case 'deleteAttachment':
                ...
                break;
            case 'addCompanyLocation':                                if ($this->isPostBack())                {                    $this->onAddLocation();                }                else                {                    $this->addLocation();                }                break;
            /* Main companies page. */
            case 'listByView':
            default:
                $this->listByView();
    ...
    private function listByView($errMessage = '')
    {
        ...
        /*          *          * ****** user access module ::: now module added to TemplateUtility********            *         $userAccessValidation = new UserAccessValidation($siteID);                $currentUserId = $_SESSION['CATS']->getUserID();                $userRoleRS = $userAccessValidation -> getUserRole($currentUserId);                $userRole = $userRoleRS[0]['role_id'];                $moduleIdRS = $userAccessValidation ->getModuleID('Companies');                $moduleId = $moduleIdRS[0]['module_schema_id'];                $moduleAccessRS = $userAccessValidation ->varifyAccess($userRole, $moduleId);                $moduleAccess = $moduleAccessRS[0]['count'];                 if($moduleAccess == '0')        {                        CommonErrors::fatal(COMMONERROR_PERMISSION, $this, 'Invalid user level for action.');        }        */
        $this->_template->assign('active', $this);
        $this->_template->assign('dataGrid', $dataGrid);
        $this->_template->assign('userID', $_SESSION['CATS']->getUserID());
        $this->_template->assign('errMessage', $errMessage);

        if (!eval(Hooks::get('CLIENTS_LIST_BY_VIEW'))) return;                //echo($_SESSION['CATS']->getUserID());
    ...
    private function show()
    {
        ...
        else
        {
            $privledgedUser = true;
                ...
                $locationsRS = $companies->getAllCompanyWorkLocation($companyID);        //echo count($locationsRS);        //exit;
        $this->_template->assign('active', $this);
        ...
        $this->_template->assign('companyID', $companyID);        $this->_template->assign('locationsRS', $locationsRS);
    ...
    private function onAdd()
    {
        ...
        /* Hot company? */
        $isHot = $this->isChecked('isHot', $_POST);                        /* Implemation Partner */        $isImpPartner = $this->isChecked('isImpPartner', $_POST);
        ...
        $companyID = $companies->add(
            ...
            $notes,$ourfee_percent, $this->_userID, $this->_userID, $isImpPartner
        );
        ...
        /*        * Called by location add popup        */        private function onAddLocation()        {                if ($this->_accessLevel < ACCESS_LEVEL_EDIT)                {                        $this->listByView('Invalid user level for action.');                        return;                    }                $operation       = $this->getTrimmedInput('operation', $_POST);        if($operation == 'edit')        {            $locationID      = $this->getTrimmedInput('locationID', $_POST);                        $companyID       = $this->getTrimmedInput('companyID', $_POST);                        $country         = $this->getTrimmedInput('country', $_POST);                        $address         = $this->getTrimmedInput('address', $_POST);                        $city            = $this->getTrimmedInput('city', $_POST);                        $state           = $this->getTrimmedInput('state', $_POST);                        $zip             = $this->getTrimmedInput('zip', $_POST);            $companies = new Companies($this->_siteID);                        $locationID = $companies->editCompanyLocation($country, $address, $city, $state, $zip, $companyID,$locationID);                        sleep(3);                        CATSUtility::transferRelativeURI('m=companies&a=show&companyID='. $companyID);                                }        elseif ($operation == 'block')        {            $locationID      = $this->getTrimmedInput('locationID', $_POST);            $companyID       = $this->getTrimmedInput('companyID', $_POST);            $flag            = '0';            $companies = new Companies($this->_siteID);            $updates = $companies->enableCompanyLocation($companyID,$locationID,$flag);            sleep(3);            CATSUtility::transferRelativeURI('m=companies&a=show&companyID='. $companyID);                    }        elseif($operation == 'unblock')        {            $locationID      = $this->getTrimmedInput('locationID', $_POST);            $companyID       = $this->getTrimmedInput('companyID', $_POST);            $flag            = '1';                        $companies = new Companies($this->_siteID);            $updates = $companies->enableCompanyLocation($companyID,$locationID,$flag);            sleep(3);            CATSUtility::transferRelativeURI('m=companies&a=show&companyID='. $companyID);        }        else        {            $companyID       = $this->getTrimmedInput('companyID', $_POST);                        $country         = $this->getTrimmedInput('country', $_POST);                        $address         = $this->getTrimmedInput('address', $_POST);                        $city            = $this->getTrimmedInput('city', $_POST);                        $state           = $this->getTrimmedInput('state', $_POST);                        $zip             = $this->getTrimmedInput('zip', $_POST);                                    $companies = new Companies($this->_siteID);                        $locationID = $companies->addCompanyLocation($country, $address, $city, $state, $zip, $companyID);                        if ($locationID <= 0)            {                CommonErrors::fatal(COMMONERROR_RECORDERROR, $this, 'Failed to add Location.');            }                        sleep(3);                        CATSUtility::transferRelativeURI('m=companies&a=show&companyID=' . $companyID);        }                      }            private function addLocation()    {                if ($this->_accessLevel < ACCESS_LEVEL_EDIT)                {                        $this->listByView('Invalid user level for action.');                        return;                    }                $companies = new Companies($this->_siteID);                $companyID = $_GET['companyID'];                $operation = $_GET['operation'];                if($operation == 'edit' || $operation == 'block' || $operation == 'unblock')        {                        $locationID = $_GET['locationID'];            $data  = $companies->getForEditLocation($locationID);            $this->_template->assign('operation', $operation);            $this->_template->assign('locationID', $locationID);            $this->_template->assign('companyID', $companyID);            $this->_template->assign('active', $this);            $this->_template->assign('data', $data);            $this->_template->display('./modules/companies/addCompanyLocation.tpl');                    }        else         {                    $this->_template->assign('companyID', $companyID);            if (!eval(Hooks::get('CLIENTS_ADD'))) return;                            $this->_template->display('./modules/companies/addCompanyLocation.tpl');        }                    }
    /*
    ...
    private function onEdit()
    {
        ...
        /* Hot company? */
        $isHot = $this->isChecked('isHot', $_POST);                $isImpPartner = $this->isChecked('isImpPartner', $_POST);
       ...
       if (!eval(Hooks::get('CLIENTS_ON_EDIT_PRE'))) return;
        ...
        if (!$companies->update($companyID, $name, $address, $city, $state,
            ...
            $isHot, $notes,$ourfee_percent, $owner, $billingContact, $email, $emailAddress, $isImpPartner))
        {
