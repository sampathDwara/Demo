...
<?php TemplateUtility::printTabs($this->active, $this->subActive); ?>
        ...
        <div id="contents">
            ...
            <form name="editUserForm" id="editUserForm" action="<?php echo(CATSUtility::getIndexName()); ?>?m=settings&amp;a=editUser" method="post" onsubmit="return checkEditUserForm(document.editUserForm);" autocomplete="off">
                ...
                <table class="editTable" width="600">
                    ...
                    <tr>                        <td class="tdVertical">User Type:</td>                        <td class="tdData">							<select tabindex="10" id="usertype" name="usertype" class="inputbox" style="width: 150px;" onchange="disableAccessLevel();">							<option value="0">General User</option>							<?php foreach ($this->getUserType as $rowNumber => $getUserTypeData): ?>															<?php if($getUserTypeData['roleId'] == $this->data['roleId']) :?>								<option selected value="<?php $this->_($getUserTypeData['roleId']) ?>"><?php $this->_($getUserTypeData['userRole']) ?></option>								<?php else:?>								<option value="<?php $this->_($getUserTypeData['roleId']) ?>"><?php $this->_($getUserTypeData['userRole']) ?></option>  								<?php endif?>															<?php endforeach; ?>							</select>&nbsp;*                        </td>                    </tr>                                <tr>                                    <td class="tdVertical">User Authentication:</td>                                    <td class="tdData">                                    	<?php $userAuths = ($this->data['userAuth']); ?>										<select tabindex="10" id="userAuth" name="userAuth" class="inputbox" style="width: 150px;" >										<option  <?php if($userAuths == 1) echo "selected"; ?>  value="1">ATS</option>										<option  <?php if($userAuths == 2) echo "selected"; ?>  value="2">Google</option>										<option  <?php if($userAuths == 3) echo "selected"; ?>  value="3">GoDaddy</option>										</select>&nbsp;*                                    </td>                                </tr>
                    <tr>
                        <td class="tdVertical">
    ...
    </div>    <script language="javascript">	function disableAccessLevel()	{		var option_str = document.getElementById('usertype');		selValue = option_str.options[option_str.selectedIndex].value;		selText = option_str.options[option_str.selectedIndex].text;				if(selText == "Recruiter")		{			document.getElementById('access400').disabled = true;			document.getElementById('access500').disabled = true;		}		else		{			document.getElementById('access400').disabled = false;			document.getElementById('access500').disabled = false;		}			}window.onload = function() {  disableAccessLevel();};		</script>	    
