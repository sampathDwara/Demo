...

{
    ...
    */
    ...
    
    {
        ...
        $getUserType = $users-> getUserType();
        
        $this->_template->assign('getUserType', $getUserType);
    ...
    */
    ...
    
    {
        ...
        $usertype   = $this->getTrimmedInput('usertype', $_POST);
        
        $userAuth   = $this->getTrimmedInput('userAuth', $_POST);
        ...
        $userID = $users->add(
            ...
            $lastName, $firstName, $email, $username, $password, $accessLevel, $usertype,$userAuth, $eeoIsVisible
    ...
    private function onAddUserType()
    {
        ...
        $userType       = $this->getTrimmedInput('userType', $_POST);
        $isActive       = $this->getTrimmedInput('isActive', $_POST);
        //$moduleid       = $this->getTrimmedInput('module', $_POST);
        
        $operation       = $this->getTrimmedInput('operation', $_POST);
        $usertypeID    = $this->getTrimmedInput('usertypeID', $_POST);
        ...
        if($operation == "edit")
        {
            $users -> editUserType($usertypeID,$userType,$isActive);
            
            if(!empty($_POST['module']))
            {
                $users -> delUserModule($usertypeID);
                
                foreach($_POST['module'] as $moduleId)
                {
                    $users -> addUserModule($usertypeID,$moduleId);
                }
            }
        }
        else
        {
            ...
            $usertypeID = $users -> addUserType($userType,$isActive);
        ...
        sleep(3);
        CATSUtility::transferRelativeURI('m=settings&a=manageUserType');
    ...
    private function addUserType()
    {
        ...
        $this->_template->assign('ModuleList', $ModuleList);
        ...
        $this->_template->assign('operation', $operation);
        ...
        if($operation == "edit")
        {
            ...
            $this->_template->assign('usertypeID', $usertypeID);
            
            $datas = $users -> getUserTypeDeatils($usertypeID);
            
            $userlistModule = $users -> getListUserModule($usertypeID);
            
            $this->_template->assign('datas', $datas);
            $this->_template->assign('userlistModule', $userlistModule);
            
        }
        
        if (!eval(Hooks::get('SETTINGS_ADD_USER'))) return;
        
        $this->_template->display('./modules/settings/UserType.tpl');
    }
    ...
    */
    ...
    
    {
        ...
        */
        ...
        if (!$users->update($userID, $lastName, $firstName, $email, $username,$usertype,$userAuth,
    ...
    */
    ...
    private function manageUserType()
    
    {
        ...
        
        {
            ...
            $roleID = $rs[$rowIndex]['roleId'];
            //$rs[$rowIndex]['moduleAccess'] = "H1";
            
            $dats = $users->getUserAccessModule($roleID);
            
            $userModules = "";
            
            foreach($dats as $rowIndex1 => $row)
            {
                if($userModules == "")
                {
                    $userModules = $dats[$rowIndex1]['moduleName'];
                }
                else
                {
                    $userModules = $userModules.', '.$dats[$rowIndex1]['moduleName'];
                }
            }
            
            $rs[$rowIndex]['moduleAccess'] = $userModules;
        ...
        $this->_template->display('./modules/settings/manageUserType.tpl');
