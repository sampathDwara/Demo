...
class JobOrdersUI extends UserInterface
{
    ...
    private function show()
    {        
        ...
        $SupplierContact = $suppliers->getSupplierContact($data['supplierContactID']);
        $SupplierContact = $SupplierContact[0]['firstName'].' '.$SupplierContact[0]['lastName'];
...
                $contact2FullName = $jobOrders -> getContactName($data['contact_id2']);        $data['contact2FullName'] = $contact2FullName[0]['ContactName'];                $client_location = $jobOrders -> getCompanyLocationName($data['client_location_id']);        $data['client_location'] = $client_location[0]['CompanyAdd'];                        $endclient_company_name = $jobOrders -> getCompanyName($data['endclient_company_id']);        $data['endclient_company_name'] = $endclient_company_name[0]['CompanyName'];                $endclient_location =  $jobOrders -> getCompanyLocationName($data['endclient_location_id']);        $data['endclient_location'] = $endclient_location[0]['CompanyAdd'];                $endclient_contact_Name = $jobOrders -> getContactName($data['endclient_contact_id']);        $data['endclient_contact_Name'] = $endclient_contact_Name[0]['ContactName'];        $endclient_contact2_Name = $jobOrders -> getContactName($data['endclient_contact_id2']);        $data['endclient_contact2_Name'] = $endclient_contact2_Name[0]['ContactName'];        
        ...
        $data['expirydate'] = DateUtility::fixZeroDate(
            $data['expirydate']
        );
        ...
        $this->_template->assign('SupplierContact', $SupplierContact);
        ...
        $this->_template->assign('clientrefno', $clientrefno);
    ...
    private function loadsuppliercontacts(){
		$suppliersContacts = new SuppliersContacts($this->_siteID);
        $suppliersRS = $suppliersContacts->getAllContacts($_GET['supplierID']); 
        $data['jocontacts']=array();
        
		foreach ($suppliersRS as $rowNumber => $suppliersData): 
		 // print_r($suppliersData);
				$key=$suppliersData['suppliercontactID'];
				$data['jocontacts'][$key]=$suppliersData['firstName']." ".$suppliersData['lastName'] ;	
    ...
    private function add()
    {
        ...
        $supplierRS = $users->getSelectSuppliersList();
        $companies = new Companies($this->_siteID);
        $companiesRS = $companies->getSelectList();
        $custCompRS = $companies->getImplemetationCompanyList();                $custCompLocRS = $companies->getCompanyWorkLocation($companyID);        
      ...
      //  $selectedCompanyID = false;
            ...
            $_GET['typeOfAdd'] == 'existing')
        {
            ...
            $jobOrderSourceRS = $jobOrders->get($jobOrderID);            $selectedCompanyID = $jobOrderSourceRS["companyID"];                        
        ...
        $this->_template->assign('selectedCompanyID', $selectedCompanyID);                //ECHO "comppid:".$selectedCompanyID;        $this->_template->assign('custCompRS', $custCompRS);                $this->_template->assign('custCompLocRS', $custCompLocRS);        
        ...
        $this->_template->assign('noSuppliers', $noSuppliers);
        $this->_template->assign('selectedSupplierID', $selectedSupplierID);
        $this->_template->assign('selectedSupplierContacts', $selectedSupplierContacts);
        $this->_template->assign('selectedSupplierLocation', $selectedSupplierLocation);
          */
    ...
    { 
        ...
        $endDate = $this->getTrimmedInput('endDate', $_POST);
        if (!empty($startDate))
        {
        ...
        $companyID   = $_POST['companyID']; // Client / Implemenation Partner Company ID
        $supplierID   = $_POST['supplierID'];
        $supplierContactsID   = $_POST['supplierContactsID'];
        ...
        $pubtitle    = $this->getTrimmedInput('pubtitle', $_POST);
        ...
        $payRate     = $this->getTrimmedInput('payRate', $_POST);
        
        
        $rate_min       = $this->getTrimmedInput('minRate', $_POST);
        $minpayRate     = $this->getTrimmedInput('minpayRate', $_POST);
        $minsalary      = $this->getTrimmedInput('minsalary', $_POST);
        $mincurrentpayRate     = $this->getTrimmedInput('mincurrentpayRate', $_POST);
        $mincurrentpayRateSal  = $this->getTrimmedInput('mincurrentpayRateSal', $_POST);      
        $ourfee_percent        = $this->getTrimmedInput('ourfee_percent', $_POST);  
        ...
        $currentpayRate  = $this->getTrimmedInput('currentpayRate', $_POST);
        $offeredpayRate       = $this->getTrimmedInput('offeredpayRate', $_POST);
        $currentpayRateSal  = $this->getTrimmedInput('currentpayRateSal', $_POST);
        $offeredpayRateSal       = $this->getTrimmedInput('offeredpayRateSal', $_POST);
        $clientbenefits       = $this->getTrimmedInput('clientbenefits', $_POST);
        $hirenotes       = $this->getTrimmedInput('hirenotes', $_POST);
        $commission_percent       = $this->getTrimmedInput('commission_percent', $_POST);
        $commission_amount       = $this->getTrimmedInput('commission_amount', $_POST);
        $benefits       = $this->getTrimmedInput('benefits', $_POST);
        $ourfee       = $this->getTrimmedInput('ourfee', $_POST);
        $pdescription       = $this->getTrimmedInput('pdescription', $_POST);
        $jobgroup = $this->getTrimmedInput('jobgroup', $_POST);                
        $partnerLocationID  = $this->getTrimmedInput('partnerLocationID', $_POST);        $contactID          = $this->getTrimmedInput('contactID', $_POST);        $contactID2         = $this->getTrimmedInput('contactID2', $_POST);                $custCompID         = $this->getTrimmedInput('custCompID', $_POST);        $WorkLocationID     = $this->getTrimmedInput('WorkLocationID', $_POST);        $clientContact1     = $this->getTrimmedInput('clientContact1', $_POST);        $clientContact2     = $this->getTrimmedInput('clientContact2', $_POST);                       $clientrefno        = $this->getTrimmedInput('clientrefno', $_POST);                
        ...
        $jobOrderID = $jobOrders->add(
            $title,$jobgroup, $pubtitle, $companyID, $contactID, $description, $notes, $duration,$minpayRate,$rate_min,$minsalary,$mincurrentpayRate,$mincurrentpayRateSal,$ourfee_percent,$maxRate, $payRate, $type, $isHot, $isPublic, $openings, $companyJobID,
            $salary, $country, $city, $state,$zip, $startDate, $endDate, $this->_userID, $recruiter,
            $owner, $department, $questionnaireID,$currentpayRate,$offeredpayRate,$benefits,$ourfee,$currentpayRateSal,$offeredpayRateSal,$clientbenefits,$hirenotes,$pdescription,$supplierID,$supplierContactsID,$commission_percent,$commission_amount,                $partnerLocationID, $contactID, $contactID2, $custCompID, $WorkLocationID, $clientContact1, $clientContact2,$clientrefno
        );
    ...
    private function edit()
    {
        ...
        $companies = new Companies($this->_siteID);
        $companiesRS = $companies->getSelectList();
        $contactsRS = $companies->getContactsArray($data['companyID']);                $endclient_contactRS = $companies->getContactsArray($data['endclient_company_id']);        $partnerLocationRS = $companies->getCompanyWorkLocation($data['companyID']);        $WorkLocationRS = $companies->getCompanyWorkLocation($data['endclient_company_id']);        
        //echo $data['endclient_company_id'];        //exit;
        $suppliers = new Suppliers($this->_siteID);
        $suppliersRS = $suppliers->getSelectList();
        $supplierContactID=$data['supplierContactID'];
        $supplierscontactsRS = $suppliers->getSupplierContactsArray($data['supplierID']);
//print_r($supplierscontactsRS);
      /*  $suppliers = new Suppliers($this->_siteID);
        $suppliersRS = $suppliers->getSelectList();
        $supplierscontactsRS = $suppliers->getSupplierContactsArray($data['companyID']);
       */
        /* Add an MRU entry. */
        $_SESSION['CATS']->getMRU()->addEntry(
            DATA_ITEM_JOBORDER, $jobOrderID, $data['title']
        ...
        $companies = new Companies($this->_siteID);
        $selectedCompanyID = $defaultCompanyID = $companies->getDefaultCompany();
        if ($defaultCompanyID !== false)
        {
            $defaultCompanyRS = $companies->get($defaultCompanyID);
        }
        ...
        if ($_SESSION['CATS']->isDateDMY())
        {
            $data['startDateMDY'] = DateUtility::convert(
             ...
             $data['endDateMDY'] = DateUtility::convert(
                '-', $data['expirydate'], DATE_FORMAT_DDMMYY, DATE_FORMAT_MMDDYY
            );
        }
        else
        {
            $data['startDateMDY'] = $data['startDate'];
            $data['endDateMDY']   = $data['expirydate'];
        }
        ...
        $this->_template->assign('commission_percent', $data['commission_percent']);
        $this->_template->assign('commission_amount', $data['commission_amount']);
    ...
    /*    
        ...
        $this->_template->assign('isHrMode', $_SESSION['CATS']->isHrMode());
        $this->_template->assign('sessionCookie', $_SESSION['CATS']->getCookie());
                $this->_template->assign('endclient_contactRS', $endclient_contactRS);        $this->_template->assign('partnerLocationRS', $partnerLocationRS);        $this->_template->assign('WorkLocationRS', $WorkLocationRS);
    ...
    private function onEdit()
    {
        ...
        if (!$jobOrders->update($jobOrderID,$jobgroup, $title, $pubtitle, $companyJobID, $companyID, $contactID,
            $description, $notes, $duration,$minpayRate,$rate_min,$minsalary,$mincurrentpayRate,$mincurrentpayRateSal,$ourfee_percent, $maxRate, $payRate, $type, $isHot, $openings, $openingsAvailable, $salary,$country, $city, $state,$zip, $startDate, $endDate, $status, $recruiter,
            $owner, $public, $email, $emailAddress, $department, $questionnaireID,$currentpayRate,$offeredpayRate,$benefits,$ourfee,$currentpayRateSal,$offeredpayRateSal,$clientbenefits,$hirenotes,$pdescription,$supplierID,$supplierContactsID,$commission_percent,$commission_amount,            $partnerLocationID,$contactID2,$custCompID,$WorkLocationID,$clientContact1,$clientContact2,$clientrefno))
        {
