...
class TemplateUtility
{
    ...
    public static function printTabs($active, $subActive = '', $forceHighlight = '')
    {
        ...
        foreach ($modules as $moduleName => $parameters)
        {
            ...
            /* If name = Suppliers and HR mode is on, change tab name to My Supplier. */
            if ($_SESSION['CATS']->isHrMode() && $tabText == 'Suppliers')
            {
                $tabText = 'My Supplier';
            }
            ...
            if ($subTabs)
            {
                ...
                foreach ($subTabs as $subTabText => $link)
                {
                    ...
                    else if (strpos($link, 'a=internalPostings') !== false)
                    {
                        ...
                        $suppliers = new Suppliers($_SESSION['CATS']->getSiteID());
