...
include_once('./lib/STDLEmail.php');
...
class LoginUI extends UserInterface
{
    ...
    private function attemptLogin()
    {
        ...
        $googleId = $this->getTrimmedInput('googleId', $_POST);
        $goDaddyId = $this->getTrimmedInput('goDaddyId', $_POST);
        ...
        if ($googleId !="") 
        {
          ...
          $userData = $users -> getUserDetailsSocial($googleId);
        ...
        elseif ($goDaddyId !="")
        {
            ...
            $users = new Users();
            $varifyEmail = new STDLEmail($siteID);
            
            
            
            $userData = $users -> getUserDetailsSocial($goDaddyId);
        ...
        */
        ...
        elseif ($userAuth == 3 )
        {
            ...
            $users = new Users();
            $STDLEmail = new STDLEmail($siteID);

            $username = $_POST['username'];
            $password = $_POST['password'];
            
            
            $varifyEmail = $STDLEmail ->varifyEmail($username, $password);
            
            //echo $varifyEmail;
            
            if($varifyEmail > 0) 
            {
                $userData = $users -> getUserDetailsSocial($username);
        ...
        $_SESSION['CATS']->processLogin($username, $password,$googleId,$goDaddyId,$userAuth);
