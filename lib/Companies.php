...
class Companies
{
    ...
    public function add($name, $address, $city, $state, $zip, $phone1,
        ...
        $notes,$ourfee_percent, $enteredBy, $owner,$isImpPartner)
    {
        $sql = sprintf(
            "INSERT INTO company (
                name,
    ...
    public function update($companyID, $name, $address, $city, $state,
                           ...
                           $keyTechnologies, $isHot, $notes,$ourfee_percent, $owner,
        $billingContact, $email, $emailAddress, $isImpPartner)
    {
        $sql = sprintf(
            "UPDATE
                company
...
                public function addCompanyLocation($country, $address, $city, $state, $zip, $companyID)            {                $sql = sprintf(                        "INSERT INTO `company_work_location`(                        `company_id`,                         `address`,                         `country`,                         `state`,                         `city`,                         `zip`,                         `date_created`)                         VALUES (                        %s,                        %s,                        %s,                        %s,                        %s,                        %s,                        now()                        )",                        $this->_db->makeQueryInteger($companyID),                        $this->_db->makeQueryString($address),            $this->_db->makeQueryString($country),                        $this->_db->makeQueryString($state),            $this->_db->makeQueryString($city),                        $this->_db->makeQueryString($zip)                        );                //echo $sql;        //exit;                $queryResult = $this->_db->query($sql);                if (!$queryResult)                {                        return -1;                    }                                $locationID = $this->_db->getLastInsertID();                                //$history = new History($this->_siteID);                //$history->storeHistoryNew(DATA_ITEM_COMPANY, $companyID);                                return $locationID;            }        public function editCompanyLocation($country, $address, $city, $state, $zip, $companyID,$locationID)        {                $sql = sprintf(                                    "UPDATE `company_work_location`              SET              `address`=%s,             `country`=%s,             `state`=%s,             `city`=%s,             `zip`=%s            WHERE             `workLocationID`=%s",                        $this->_db->makeQueryString($address),                        $this->_db->makeQueryString($country),                        $this->_db->makeQueryString($state),                        $this->_db->makeQueryString($city),                        $this->_db->makeQueryString($zip),            $this->_db->makeQueryInteger($locationID)                        );        //echo $sql;       //exit;                $queryResult = $this->_db->query($sql);                //if (!$queryResult)                {                        //return -1;                    }            }    public function enableCompanyLocation($companyID,$locationID,$flag)        {                $sql = sprintf(                                    "UPDATE `company_work_location`             SET             `is_active`=%s            WHERE            `workLocationID`=%s",                        $this->_db->makeQueryString($flag),                        $this->_db->makeQueryInteger($locationID)                        );                        $this->_db->query($sql);            }                        public function getForEditLocation($locationID)        {                $sql = sprintf(                        "SELECT                            company_work_location.address AS address,                            company_work_location.city AS city,                            company_work_location.state AS state,                            company_work_location.zip AS zip,                            company_work_location.country AS country,                company_work_location.is_active AS active                        FROM                            company_work_location                        WHERE                            company_work_location.workLocationID = %s",                        $this->_db->makeQueryInteger($locationID)                        );                                return $this->_db->getAssoc($sql);            }    
    /**
    ...
    public function get($companyID)
    {
        $sql = sprintf(
            "SELECT
                ...
                company.ourfee_percent AS ourfee_percent,
                ...
                owner_user.email AS owner_email,                company.isImpPartner AS isImpPartner
            FROM
                company
...
     /**          * Returns a minimal record set of all companies (for use when creating          * drop-down lists of companies, etc.).          *          * @return array Implemetation Companies data          */          public function getImplemetationCompanyList()          {                  $sql = sprintf(                          "SELECT                             company.company_id AS implCompanyID,                             company.name AS CompanyName                         FROM                             company                         WHERE                             company.site_id = %s                             AND                             company.isImpPartner = 1                                      ORDER BY                             company.name ASC",                          $this->_siteID                          );                           return $this->_db->getAllAssoc($sql);              }       public function getCompanyWorkLocation($companyID)          {         //$companyID= 17928;                  $sql = sprintf(                          "SELECT                             company.company_id AS implCompanyID,                company.name AS CompanyName,                company_work_location.workLocationID AS CompLocID,                 CONCAT(                    company_work_location.address, ', ',company_work_location.country, ', ', company_work_location.state,', ',company_work_location.city,', ',company_work_location.zip                 ) AS CompanyAdd,                company_work_location.address AS address,                 company_work_location.country AS country,                company_work_location.state AS state,                company_work_location.city AS city,                company_work_location.zip AS zip,                company_work_location.date_created AS date_created,                company_work_location.is_active AS active                            FROM                             company            LEFT JOIN company_work_location                ON company.company_id = company_work_location.company_id                         WHERE                             company.site_id = %s                             AND                             company_work_location.company_id = %s                 AND                             company_work_location.is_active = '1'                          ORDER BY                             company.name ASC",                          $this->_siteID,             $this->_db->makeQueryInteger($companyID)                          );                  //echo $sql;         //exit;         return $this->_db->getAllAssoc($sql);              }
     public function getAllCompanyWorkLocation($companyID)          {         //$companyID= 17928;                  $sql = sprintf(                          "SELECT                             company.company_id AS implCompanyID,                company.name AS CompanyName,                company_work_location.workLocationID AS CompLocID,                CONCAT(                                 company_work_location.address, ', ',company_work_location.country, ', ', company_work_location.state,', ',company_work_location.city,', ',company_work_location.zip                             ) AS CompanyAdd,                             company_work_location.address AS address,                company_work_location.country AS country,                company_work_location.state AS state,                company_work_location.city AS city,                company_work_location.zip AS zip,                company_work_location.date_created AS date_created,                company_work_location.is_active AS active                         FROM                             company                         LEFT JOIN company_work_location                             ON company.company_id = company_work_location.company_id                         WHERE                             company.site_id = %s                             AND                             company_work_location.company_id = %s                                                   ORDER BY                             company.name ASC",                          $this->_siteID,             $this->_db->makeQueryInteger($companyID)                          );                  //echo $sql;         //exit;         return $this->_db->getAllAssoc($sql);              }                    /**
...
     public function getJOLocationArray($companyID)          {         $sql = sprintf(             "SELECT                 	company_work_location.workLocationID AS LocationID,                    CONCAT(company_work_location.address,', ',company_work_location.country,', ',company_work_location.state,', ',company_work_location.city,', ',company_work_location.zip )AS LocAddress,                	                	company.ourfee_percent AS ourfee_percent                FROM                 	company_work_location                LEFT JOIN company                	ON company.company_id = company_work_location.company_id                WHERE                 	company_work_location.company_id = %s                AND                	company.site_id = %s",                          $this->_db->makeQueryInteger($companyID),                          $this->_siteID                          );                           return $this->_db->getAssoc($sql);              }               
    /**
...
     public function CheckCompanyImpPartner($companyID)     {         $sql = "SELECT `isImpPartner` AS `isImpPartner` FROM `company` WHERE `company_id` =".$companyID;         return $this->_db->getAssoc($sql);     }
    /**
