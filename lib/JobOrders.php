...
class JobOrders
{
     ...
     */
    public function add($title,$jobgroup,$pubtitle, $companyID, $contactID, $description, $notes,
        $duration, $minpayRate,$rate_min,$minsalary,$mincurrentpayRate,$mincurrentpayRateSal,$ourfee_percent, $maxRate,$payRate, $type, $isHot, $public, $openings, $companyJobID,
        $salary,$country, $city, $state,$zip, $startDate, $endDate, $enteredBy, $recruiter, $owner,
        $department, $questionnaire = false,$currentpayRate,$offeredpayRate,$benefits,$ourfee,$currentpayRateSal,$offeredpayRateSal,$clientbenefits,$hirenotes,$pdescription,$supplierID,$supplierContactsID,$commission_percent,$commission_amount,                $partnerLocationID, $contactID, $contactID2, $custCompID, $WorkLocationID, $clientContact1, $clientContact2, $clientrefno)
    {
        ...
        $sql = sprintf(
            "INSERT INTO joborder  SET 
                ...
                commission_amount = %s
           ",
            $this->_db->makeQueryString($title),                        $this->_db->makeQueryString($clientrefno),
            $this->_db->makeQueryString($companyJobID),
            $this->_db->makeQueryInteger($companyID),
            $this->_db->makeQueryInteger($contactID),            $this->_db->makeQueryInteger($contactID2),            $this->_db->makeQueryInteger($partnerLocationID),            $this->_db->makeQueryInteger($custCompID),            $this->_db->makeQueryInteger($WorkLocationID),            $this->_db->makeQueryInteger($clientContact1),            $this->_db->makeQueryInteger($clientContact2),                        
            $this->_db->makeQueryString($description),
            $this->_db->makeQueryString($notes),
            $this->_db->makeQueryString($duration),
            $this->_db->makeQueryString($minpayRate),
            $this->_db->makeQueryString($rate_min),
            $this->_db->makeQueryString($minsalary),
            $this->_db->makeQueryString($mincurrentpayRate),
            $this->_db->makeQueryString($mincurrentpayRateSal),
            
            $this->_db->makeQueryString($ourfee_percent),
            $this->_db->makeQueryString($maxRate),
            $this->_db->makeQueryString($payRate),
            ...
            $this->_db->makeQueryString($currentpayRate),
            $this->_db->makeQueryString($offeredpayRate),
            $this->_db->makeQueryString($benefits),
            $this->_db->makeQueryString($ourfee),
            $this->_db->makeQueryString($currentpayRateSal),
            $this->_db->makeQueryString($offeredpayRateSal),
            $this->_db->makeQueryString($clientbenefits),
            $this->_db->makeQueryString($hirenotes),
            $this->_db->makeQueryString($pdescription),
            $this->_db->makeQueryInteger($supplierID),
            $this->_db->makeQueryString($pubtitle),
            $this->_db->makeQueryInteger($supplierContactsID),
            $this->_db->makeQueryString($jobgroup),
            $this->_db->makeQueryInteger($commission_percent),
            $this->_db->makeQueryString($commission_amount)            
        );
    ...
    public function update($jobOrderID,$jobgroup, $title, $pubtitle, $companyJobID, $companyID,
        $contactID, $description, $notes, $duration, $minpayRate,$rate_min,$minsalary,$mincurrentpayRate,$mincurrentpayRateSal,$ourfee_percent, $maxRate, $payRate, $type, $isHot,$openings, $openingsAvailable, $salary, $country, $city, $state,$zip, $startDate, $endDate, $status, $recruiter,
        $owner, $public, $email, $emailAddress, $department, $questionnaire = false,$currentpayRate,$offeredpayRate,$benefits,$ourfee,$currentpayRateSal,$offeredpayRateSal,$clientbenefits,$hirenotes,$pdescription,$supplierID,$supplierContactsID,$commission_percent,$commission_amount,                $partnerLocationID,$contactID2,$custCompID,$WorkLocationID,$clientContact1,$clientContact2, $clientrefno)
    { 
        /* Get the department ID of the selected department. */
        // FIXME: Move this up to the UserInterface level. I don't like this
        //        tight coupling, and calling Contacts methods as static is
        ...
        // FIXME: Is the OrNULL usage below correct? Can these fields be NULL?
        //ALTER TABLE `joborder` CHANGE `ourfee_precent` `ourfee_percent` VARCHAR(60) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT '0';
        $sql = sprintf(
            "UPDATE
                joborder
             SET
            ...
            $this->_db->makeQueryString($hirenotes),
            $this->_db->makeQueryString($pdescription),
            ($public ? '1' : '0'),         // Questionnaire ID or NULL if none
            $questionnaire !== false ? $this->_db->makeQueryInteger($questionnaire) : 'NULL',
            $this->_db->makeQueryString($jobgroup),
    ...
    public function get($jobOrderID)
    {
        $sql = sprintf(
            "SELECT
                ...
                joborder.contact_id AS contactID,                joborder.supplier_id AS supplierID,
                joborder.jobgroup AS jobgroup,
                joborder.suppliercontact_id AS supplierContactID,
                joborder.client_job_id AS companyJobID,
                joborder.title AS title,                joborder.client_ref_no AS clientrefno,
                joborder.pubtitle AS pubtitle,
                ...
                joborder.rate_pay AS payRate,
                
                joborder.rate_min AS minRate,
                joborder.minpayRate AS minpayRate,
                joborder.minsalary AS minsalary,
                joborder.mincurrentpayRate AS mincurrentpayRate,
                joborder.mincurrentpayRateSal AS mincurrentpayRateSal,
                joborder.ourfee_percent AS ourfee_percent,
                ...
                joborder.currentpayRate AS currentpayRate,                
                joborder.offeredpayRate AS offeredpayRate,
                joborder.benefits AS benefits,
                joborder.ourfee AS ourfee,
                
                joborder.currentpayRateSal AS currentpayRateSal,
                
                joborder.offeredpayRateSal AS offeredpayRateSal,
                
                joborder.commission_percent AS commission_percent,
                joborder.commission_amount AS commission_amount,
                joborder.benefits AS clientbenefits,
                joborder.hirenotes AS hirenotes,
                joborder.pdescription as pdescription,
                ...
                company.name AS companyName,                joborder.contact_id2 AS contact_id2,                joborder.client_location_id AS client_location_id,                joborder.endclient_company_id AS endclient_company_id,                joborder.endclient_location_id AS endclient_location_id,                joborder.endclient_contact_id AS endclient_contact_id,                joborder.endclient_contact_id2 AS endclient_contact_id2
            FROM
                joborder
            LEFT JOIN company
...
if ($_SESSION['CATS']->isLoggedIn() && $_SESSION['CATS']->getAccessLevel() ==370 )
        {
            $recruiterHiddenCriterion = 'AND joborder.recruiter = '.$_SESSION['CATS']->getUserID()." ";
        }
        else
        {
            $recruiterHiddenCriterion = '';
        }
        if ($companyID >= 0)
        {
...
        public function getCompanyName($companyID)    {        $sql = sprintf(                        "SELECT                company.name AS CompanyName            FROM                company            WHERE                company.company_id = %s",            $this->_db->makeQueryInteger($companyID)                        );        return $this->_db->getAllAssoc($sql);    }    public function getCompanyLocationName($locationID)    {        $sql = sprintf(                        "SELECT                CONCAT(                    company_work_location.address, ', ',company_work_location.country, ', ', company_work_location.state,', ',company_work_location.city,', ',company_work_location.zip                 ) AS CompanyAdd            FROM                company_work_location            WHERE                company_work_location.workLocationID = %s",            $this->_db->makeQueryInteger($locationID)                        );        return $this->_db->getAllAssoc($sql);    }                   public function getContactName($contactID)    {        $sql = sprintf(                        "SELECT                        CONCAT( contact.first_name,' ', contact.last_name) AS ContactName                        FROM                            contact                        WHERE                            contact.contact_id = %s",                        $this->_db->makeQueryInteger($contactID)            );                                return $this->_db->getAllAssoc($sql);    }    
    /**
...
class JobOrdersDataGrid extends DataGrid
{
    ...
    public function __construct($instanceName, $siteID, $parameters, $misc)
    {        //echo ($parameters);        
        ...
        $this->_classColumns = array(
            ...
            'Industry' =>        array('select'         => 'joborder.jobgroup AS jobgroup',
                                      'pagerRender'    => 'if ($rsData[\'isHot\'] == 1) $className =  \'jobLinkHot\'; else $className = \'jobLinkCold\'; return \'<a href="'.CATSUtility::getIndexName().'?m=joborders&amp;a=show&amp;jobOrderID=\'.$rsData[\'jobOrderID\'].\'" class="\'.$className.\'">\'.htmlspecialchars($rsData[\'jobgroup\']).\'</a>\';',
                                      'sortableColumn' => 'jobgroup',
                                      'pagerWidth'     => 30,
                                      'pagerOptional'  => false,
                                      'alphaNavigation'=> true,
                                      'filter'         => 'joborder.jobgroup'),
            'Title' =>        array('select'         => 'joborder.title AS title',
                                      ...
                                      'pagerOptional'  => false,
                                      'alphaNavigation'=> true,
                                      'filter'         => 'joborder.title'),                        'Client Ref.No'  => array ('select' => 'joborder.client_ref_no AS client_ref_no',                                                        'sortableColumn' => 'client_ref_no',                                                                                'pagerWidth'     => 65,                                                                                'pagerOptional'  => true,                                                                                'alphaNavigation'=> false,                                                                                'exportColumnHeaderText' => 'Client Ref.No',                                                                                'columnHeaderText' => 'Client Ref.No',                                                                                'filter'         => 'joborder.client_job_id',                                                                                'filterTypes'   => '===>=<'),
            'Client' =>       array('select'         => 'company.name AS companyName, company.company_id AS companyID',
                                      ...
                                      'pagerOptional'  => true,
                                      'alphaNavigation'=> true,
                                      'filter'         => 'company.name'),            'EndClient' =>       array('select'         => 'clientcompany.name AS endClient,joborder.endclient_company_id AS endClientId,CONCAT(company_work_location.address,SPACE(3),company_work_location.city, SPACE(3),company_work_location.state,SPACE(3), company_work_location.country,SPACE(3), company_work_location.zip) AS location ',                                                    'pagerRender'    => 'return \'<a href="'.CATSUtility::getIndexName().'?m=companies&amp;a=show&amp;companyID=\'.$rsData[\'endClientId\'].\'" title="\'.$rsData[\'location\'].\'">\'.htmlspecialchars($rsData[\'endClient\']).\'</a>\';',                                                                        'sortableColumn' => 'EndClient',                                                                        'pagerWidth'     => 125,                                                                        'pagerOptional'  => true,                                                                        'alphaNavigation'=> true,                                                                        'filter'         => 'clientcompany.name'),
            ...
            'Created' =>       array('select'   => 'DATE_FORMAT(joborder.date_created, \'%m-%d-%y\') AS dateCreated',
                                     ...
                                     'pagerWidth'    => 60,
                                     'filterHaving' => 'DATE_FORMAT(joborder.expirydate, \'%m-%d-%y\')'),
                                     
            'Start Date' =>       array('select'   => 'DATE_FORMAT(joborder.start_date, \'%m-%d-%y\') AS start_date',
                                     'pagerRender'      => 'return $rsData[\'start_date\'];',
                                     'sortableColumn'     => 'dateCreatedSort',
                                     'pagerWidth'    => 60,
                                     'filterHaving' => 'DATE_FORMAT(joborder.start_date, \'%m-%d-%y\')'),
            
            'Expiry Date' =>       array('select'   => 'DATE_FORMAT(joborder.expirydate, \'%m-%d-%y\') AS expirydate',
                                     'pagerRender'      => 'return $rsData[\'expirydate\'];',
                                     'sortableColumn'     => 'dateCreatedSort',
                                     'pagerWidth'    => 60,
                                     'filterHaving' => 'DATE_FORMAT(joborder.expirydate, \'%m-%d-%y\')'),                         
            ...
            'Zip'          => array('select'   => 'joborder.zip AS locationzip',
                                     'pagerRender'      => 'return $rsData[\'locationzip\'];',
                                     'exportRender'     => 'return $rsData[\'locationzip\'];',
                                     'sortableColumn'     => 'locationzip',
                                     'pagerWidth'    => 65,
                                     'alphaNavigation' => true,
                                     'filter'         => 'joborder.zip'),                         

            'Bill Rate'          => array('select'   => 'joborder.rate_max AS maxRate',
                                     ...
                                     'filterTypes'   => '===>=<'),
           'Pay Rate'          => array('select'   => 'joborder.rate_pay AS payRate',
                                     'pagerRender'      => 'return $rsData[\'payRate\'];',
                                     'exportRender'     => 'return $rsData[\'payRate\'];',
                                     'sortableColumn'     => 'payRate',
                                     'pagerWidth'    => 85,
                                     'alphaNavigation' => false,
                                     'filter'         => 'joborder.rate_pay',
                                     'filterTypes'   => '===>=<'),                          
    ...
    public function getSQL($selectSQL, $joinSQL, $whereSQL, $havingSQL, $orderSQL, $limitSQL, $distinct = '')
    {        //echo "TEST::".$selectSQL;
        ...
        $sql = sprintf(
            ...
            FROM
                joborder
            LEFT JOIN company
                ON joborder.company_id = company.company_id           LEFT JOIN company AS clientcompany                  ON joborder.endclient_company_id = clientcompany.company_id  	       LEFT JOIN company_work_location       		  ON joborder.endclient_location_id = company_work_location.workLocationID            LEFT JOIN contact
                ON joborder.contact_id = contact.contact_id
            LEFT JOIN attachment
 ...
         //echo '<br>'.$sql;        //echo "<br>TEST::".(strlen($whereSQL) > 0) ? ' AND ' . $whereSQL : '';
        return $sql;
    }
}
