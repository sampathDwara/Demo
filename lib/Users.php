...

{
    ...
    public function add($lastName, $firstName, $email, $username, $password,
        ...
        $accessLevel, $usertype,$userAuth, $eeoIsVisible = false, $userSiteID = -1)

    {
        ...
        $this->_db->makeQueryInteger($usertype),
            
        $this->_db->makeQueryInteger($userAuth),    
    ...
    public function update($userID, $lastName, $firstName, $email,
        ...
        $username,$usertype, $userAuth, $accessLevel = -1, $eeoIsVisible = false)

    {
        ...
        $sql = sprintf(
                ...
                $this->_db->makeQueryString($usertype),
            
                $this->_db->makeQueryString($userAuth),
...

    {
        ...
        $sql = sprintf(
                ...
                DATE_FORMAT(
                    ...
                    ON user.user_role_id = user_role.user_role_id
    ...
    public function getUserAccessModule($roleID)
    
    {
        
        $sql = "SELECT 
                `user_role_module`.`role_module_id` AS `role_module_id`, 
                `user_role_module`.`user_role_id` AS `user_role_id`, 
                `user_role_module`.`module_schema_id` AS `module_schema_id`, 
                `user_role_module`.`is_active` AS `is_active`,
                CONCAT(UCASE(LEFT(`module_schema`.`name`, 1)), SUBSTRING(`module_schema`.`name`, 2)) AS `moduleName`
                FROM 
                `user_role_module` 
                LEFT JOIN
                `module_schema`
                on  
                `user_role_module`.`module_schema_id` = `module_schema` .`module_schema_id`
                where
                `user_role_module`.`user_role_id` =".$roleID;
    ...
    
    {
        ...
        $sql = "SELECT 
                    `module_schema`.`module_schema_id` AS `module_schema_id`, 
                    `module_schema`.`name` AS `module_name`
                 FROM 
                    `module_schema` ";
    ...
    public function getUserTypeDeatils($userID)
    {
        $sql = "SELECT 
                `user_role`.`user_role_id` AS `roleID`, 
                `user_role`.`user_role` AS `roleName`, 
                `user_role`.`is_active` AS 'isActive'
                FROM 
                `user_role` 
                WHERE 
                `user_role`.`user_role_id` = ".$userID;
        
        return $this->_db->getAllAssoc($sql);
    }
  
    public function getListUserModule($usertypeID)
    {
        $sql = "SELECT
                `user_role_module`.`module_schema_id`,
                `module_schema`.`name`
                FROM
                `user_role_module`
                LEFT JOIN
                ...
                `user_role_module`.`module_schema_id` = `module_schema` .`module_schema_id`
                where
                `user_role_module`.`user_role_id`= ".$usertypeID;
        
        return $this->_db->getAllAssoc($sql);
    }
    
    
    public function addUserType($userType,$isActive)
    {
        $sql = "INSERT INTO `user_role`(`user_role`, `is_active`) VALUES ('".$userType."',".$isActive.")";
    ...
    public function editUserType($userTypeId,$userType,$isActive)
    {
        $sql = "UPDATE `user_role` SET `user_role`='".$userType."',`is_active`=".$isActive." WHERE `user_role_id`=".$userTypeId;
        $queryResult = $this->_db->query($sql);
    }
    
    public function addUserModule($userTypeId,$moduleId)
    {
        $sql = "INSERT INTO `user_role_module`(`user_role_id`, `module_schema_id`, `is_active`) VALUES (".$userTypeId.",".$moduleId.",1)";
        $queryResult = $this->_db->query($sql);
    }
    
    public function delUserModule($userTypeId)
    {
        $sql = "DELETE FROM `user_role_module` WHERE `user_role_id` =".$userTypeId;
        $queryResult = $this->_db->query($sql);
    }
    /**
    ...
    public function getSelectSuppliersList()
    ...
    public function getSelectRecruitersList()
    ...
    public function isGoogleLogin($username, $password)
    {
        $existsInLDAP = false;
        $existsInDB = false;
    ...
    public function isGoDaddyLogin($username, $password)
    {
        $existsInLDAP = false;
        $existsInDB = false;
    ...
    public function getUserDetailsSocial($googleId)
    {
        $sql = "select user_name AS username,password AS password from user where email='".$googleId."'";
        return $this->_db->getAllAssoc($sql);
    ...
    public function getUserDetailsAuth($username)
    {
        $sql = "select user_auth AS userAuth from user where user_name='".$username."'";
        return $this->_db->getAllAssoc($sql);
        //`return ($rs['password'] == LDAPUSER_PASSWORD);
    }
    /**
