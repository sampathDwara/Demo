...
class SearchCandidates
{
    ...
    public function byKeySkills($wildCardString, $sortBy, $sortDirection)
    {
        ...
        $wildCardString = str_replace('*', '%', $wildCardString);
        $wildCardString = strtolower($this->_db->makeQueryString($wildCardString));
        ...
        $sql = sprintf(
            ...
            LEFT JOIN user AS owner_user
                ON candidate.owner = owner_user.user_id
            WHERE
               (LOWER(candidate.key_skills) LIKE |%s| OR LOWER(candidate.technology) LIKE |%s|  OR LOWER(candidate.technology_1) LIKE |%s|  OR LOWER(candidate.technology_2) LIKE |%s|)
            AND
                candidate.is_admin_hidden = 0
            AND
                candidate.site_id = %s
      ...
      /*  $where1 = DatabaseSearch::makeBooleanSQLWhere(
        ...
        $sql = sprintf(
            ...
            LEFT JOIN user AS owner_user
                ON candidate.owner = owner_user.user_id
            WHERE
                (LOWER(candidate.city) LIKE |%s| OR LOWER(candidate.address) LIKE |%s|)
            AND
                candidate.is_admin_hidden = 0
            AND
                candidate.site_id = %s
 ...
 *	@subpackage Library
 */
class SearchImplemenationPartner
{
    private $_db;
    private $_siteID;
    protected $_userID = -1;
...
class SearchSuppliers
{
    ...
    public function byName($wildCardString, $sortBy, $sortDirection)
    {
        ...
        $sql = sprintf(
            "SELECT
                ...
                DATE_FORMAT(
                    supplier.date_created, '%%m-%%d-%%y'
                ) AS dateCreated,
                DATE_FORMAT(
                    supplier.date_modified, '%%m-%%d-%%y'
                ) AS dateModified,
                owner_user.first_name AS ownerFirstName,
                owner_user.last_name AS ownerLastName
            FROM
        ...
        //echo $sql;
        //exit;
        $returnarray = $this->_db->getAllAssoc($sql);
        return $returnarray;
    }
    ...
    {
        $WHERE = DatabaseSearch::makeBooleanSQLWhere(
            $wildCardString, $this->_db, 'supplier.key_technologies'
        );
...
class SearchJobOrders
{
    ...
    public function recentlyModified($sortDirection, $activeOnly, $limit)
    {
        ...
        $sql = sprintf(
            "SELECT
                company.company_id AS companyID,
                company.name AS companyName,					 joborder.endclient_company_id as endClientId,                (select c.name from company c where c.company_id = joborder.endclient_company_id) AS endClient,                								joborder.client_ref_no AS Client_Ref_No,								joborder.contact_id AS contactID,				(select concat(c1. first_name,' ',c1.last_name) from contact c1 where c1.contact_id = joborder.contact_id) AS Primary_Contact,
                												joborder.title AS title,
                joborder.type AS type,
                joborder.is_hot AS isHot,
                joborder.duration AS duration,
...
class ContactsSearch
{
 ...
 /**
    ...
    public function byCity($wildCardString, $sortBy, $sortDirection)
    {
    	$wildCardString = strtolower($wildCardString) ;
        $wildCardString = str_replace('*', '%', $wildCardString) . '%';
        $wildCardString = $this->_db->makeQueryString($wildCardString);
