...
class CATSSession
{
     ...
     * @return void
     */
    public function processLogin($username, $password,$googleId,$goDaddyId,$userAuth, $addToHistory = true)
    {
        ...
        if ($googleId !="")
        {
            $loginStatus = $users->isGoogleLogin($username, $password);
        }
        elseif ($goDaddyId!="")
        {
            $loginStatus = $users->isGoDaddyLogin($username, $password);
        }
        else 
        {
        ...
        */
        ...
        $userAuthArray = $users -> getUserDetailsAuth($username);
        
        $userAuthDeatis = $userAuthArray[0]['userAuth'];
        
        IF($userAuthDeatis != $userAuth)
        {
            if($userAuthDeatis == 2){
                $loginStatus = "GOOGLE_LOGIN";
            }
            elseif($userAuthDeatis == 3){
                $loginStatus = "GODADDY_LOGIN";
            }
            else {
                $loginStatus = "ATS_LOGIN";
            }
        }
        
        //echo $userAuthDeatis."=".$userAuth;
        //exit;
        ...
        switch ($loginStatus)
        {
            ...
            case ATS_LOGIN:
                $this->_isLoggedIn = false;
                $this->_loginError = 'Kindly login by using ATS credential.';
                break;

            case GOOGLE_LOGIN:
                $this->_isLoggedIn = false;
                $this->_loginError = 'Kindly login by using your GOOGLE credential.';
                break;
                
            case GODADDY_LOGIN:
                $this->_isLoggedIn = false;
                $this->_loginError = 'Kindly login by using your GoDADDY credential.';
                break;
