...
class ActivityEntries
{
        ...
        $jobOrderID = false, $date = false, $timezoneOffset)
    {
     ...
     /*   $sql = sprintf(
        ...
        $sql =
            "UPDATE activity SET
                joborder_id  ='".$this->_db->makeQueryInteger($jobOrderID)."',
	            type = '".$this->_db->makeQueryInteger($activityType)."',
	            notes = '".$this->_db->makeQueryString($activityNotes)."',	            
                date_modified = NOW(),
                contact_besttime= '".$contact_besttime."',
	            contact_location = '".$contact_location."',
	            contact_preferred_traveltime= '".$contact_preferred_traveltime."',
	            contact_typing_speed = '".$contact_typing_speed."',
	            contact_pay_current = '".$contact_pay_current."',
	            contact_pay_expectation = '".$contact_pay_expectation."',
	            contact_howsoonstart = '".$contact_howsoonstart."',
	            contact_keyskills = '".$contact_keyskills."',
	            contact_otherdetails = '".$contact_otherdetails."',
	            confirmedconfirmedpayrate = '".$confirmedconfirmedpayrate."',
	            interviewschedulerequested = '".$interviewschedulerequested."',
	            interviewschedulecustomMessage = '".$interviewschedulecustomMessage."',
	            interviewconfirmeddate = '".$interviewconfirmeddate."',
	            interviewconfirmedcustomMessage = '".$interviewconfirmedcustomMessage."',
	            insterviewlostreason = '".$insterviewlostreason."',
	            candidatestartdate = '".$candidatestartdate."',
	            candidateplaceddate = '".$candidateplaceddate."',
	            supplierID = '".$supplierID."',
	            supplierContactsID = '".$supplierContactsID."'
            WHERE  WHERE
                activity_id = '".$this->_db->makeQueryInteger($activityID)."'
            AND
    ...
    public function getAllByDataItem($dataItemID, $dataItemType)
    {
        $sql = sprintf(
            "SELECT
                ...
                joborder.title AS regardingJobTitle,
                company.name AS regardingCompanyName,
                activity.contact_pay_expectation,
                activity.contact_pay_current
            FROM
                activity
            LEFT JOIN user AS entered_by_user
                ON activity.entered_by = entered_by_user.user_id
